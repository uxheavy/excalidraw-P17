...
| [`onElementActivate`](#onelementactivate) | `function` | \_ | Called when a native element is activated; return `true` to consume the activation. |
| [`isElementTextEditable`](#iselementtexteditable) | `function` | `true` | Controls whether a native element may enter text editing. |
...
| [`renderHostElement`](/docs/@excalidraw/excalidraw/api/props/render-props#renderHostElement) | `function` | \_ | Render host-owned content for non-URL native elements |
...
This callback is invoked when a native element is activated by pointer or keyboard input.

```ts
(element: NonDeletedExcalidrawElement) => boolean;
```

Return `true` to consume the activation and prevent Excalidraw's default native activation behavior. Return `false`, or omit the callback, to preserve the default behavior.

### isElementTextEditable

This callback controls whether a native element may enter text editing.

```ts
(element: NonDeletedExcalidrawElement) => boolean;
```

When omitted, the callback defaults to `true` for every element. Excalidraw still applies its usual structural checks for whether the element supports text editing.
