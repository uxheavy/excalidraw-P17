...
Renders host-owned content over visible non-URL native elements. Return `null` for elements that the host does not own. Elements with links and iframe-like types are never passed to this callback, so host content stays outside URL and iframe interaction semantics while the native element remains responsible for selection, transforms, bindings, ordering, history, and clipboard behavior.
